...
test("setup reports ready when fake Gemini is installed and authenticated", () => {
  ...
  assert.match(payload.gemini.detail, /headless mode available/);
...
test("adversarial review renders structured findings over app-server turn/start", () => {
  ...
  assert.equal(fakeState.lastInvocation.promptFlag, true);
  assert.equal(fakeState.lastInvocation.positionalPrompt, false);
  assert.match(
    fakeState.lastInvocation.prompt,
...
test("review includes reasoning output when the app server returns it", () => {
  ...
  assert.match(
    result.stdout,
    /Inspected the prompt, gathered evidence, and checked the highest-risk paths first/i,
  );
});
...
test("task --resume-last resumes the latest persisted task thread", () => {
  ...
  const fakeState = JSON.parse(
    fs.readFileSync(path.join(binDir, "fake-gemini-state.json"), "utf8"),
  );
  assert.equal(fakeState.lastInvocation.resume, fakeResumeId(1));
  assert.equal(fakeState.lastInvocation.promptFlag, true);
  assert.equal(fakeState.lastInvocation.positionalPrompt, false);
});

test("task --resume-last fails closed when plugin job state is missing", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  const stateDir = resolveStateDir(repo);
  installFakeGemini(binDir);
  ...
  // Should fail closed instead of falling back to global-latest session
  const result = run("node", [SCRIPT, "task", "--resume-last", "follow up"], {
    cwd: repo,
    env: buildEnv(binDir),
  });
  ...
  assert.ok(
    result.stdout.includes("No previous Gemini task session") ||
      result.stderr.includes("No previous Gemini task session"),
  );
});
...
test("task --resume acts like --resume-last without leaking the flag into the prompt", () => {
  ...
  assert.equal(fakeState.lastInvocation.sessionId, "thr_1");
  assert.equal(fakeState.lastInvocation.resume, fakeResumeId(1));
  assert.equal(fakeState.lastInvocation.promptFlag, true);
  assert.equal(fakeState.lastInvocation.positionalPrompt, false);
  assert.equal(fakeState.lastInvocation.prompt, "follow up");
});
...
test("task --fresh is treated as routing control and does not leak into the prompt", () => {
  ...
  assert.equal(fakeState.lastInvocation.promptFlag, true);
  assert.equal(fakeState.lastInvocation.positionalPrompt, false);
  assert.equal(fakeState.lastInvocation.prompt, "diagnose the flaky test");
});
...
test("task forwards model selection and reasoning effort to app-server turn/start", () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  const fakeState = JSON.parse(fs.readFileSync(statePath, "utf8"));
  assert.equal(fakeState.lastInvocation.model, "flash");
  assert.equal(fakeState.lastInvocation.approvalMode, "plan");
  assert.equal(fakeState.lastInvocation.promptFlag, true);
});

test("task preserves Gemini CLI default model when the user does not pass --model", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  const statePath = path.join(binDir, "fake-gemini-state.json");
  installFakeGemini(binDir);
  ...
  assert.equal(result.status, 0, result.stderr);
  const fakeState = JSON.parse(fs.readFileSync(statePath, "utf8"));
  assert.equal(fakeState.lastInvocation.model, null);
});
...
test("task with no subagent telemetry still logs the final Gemini output", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  ...
  const log = fs.readFileSync(state.jobs[0].logFile, "utf8");
  assert.match(log, /Starting Gemini Task\./);
  assert.match(log, /Final output/);
  assert.match(log, /Handled the requested task\./);
});
...
test("task using the shared broker still completes when Gemini spawns subagents", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  installFakeGemini(binDir, "with-subagent");
  initGitRepo(repo);
...
test("review accepts --background while still running as a tracked review job", () => {
  ...
  assert.match(launchPayload.provider.stdout, /No material issues found/);
...
test("status shows phases, hints, and the latest finished job", () => {
  ...
  const { GEMINI_COMPANION_SESSION_ID: _ignored, ...cleanEnv } = process.env;
  ...
  assert.match(result.stdout, /Session runtime: Gemini direct startup/);
  ...
  assert.match(
    result.stdout,
    /Resume: gemini --resume 00000000-0000-4000-8000-000000000002/,
  );
});
...
test("status preserves adversarial review kind labels", () => {
  ...
  assert.match(
    result.stdout,
    /- review-adv \| completed \| adversarial-review \| Gemini Adversarial Review/,
  );
...
test("result for a finished write-capable task returns the raw Gemini final response", () => {
  ...
  assert.match(
    result.stdout,
    /Resume: gemini --resume 00000000-0000-4000-8000-\d{12}/,
  );
});
...
test("cancel reports no provider turn interrupt for direct Gemini jobs before killing the task", async () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  ...
  assert.match(runningJob.threadId, /^thr_/);
...
test("session end fully cleans up jobs for the ending session", async (t) => {
  ...
  // All job files are preserved when orphaned (not deleted)
  assert.deepEqual(
  ...
  // Jobs are orphaned (sessionId cleared) rather than deleted to preserve results
  assert.deepEqual(
    state.jobs.map((job) => job.id).sort(),
    ["review-completed", "review-other", "review-running"].sort(),
  );
  const otherJob = state.jobs.find((job) => job.id === "review-other");
  assert.equal(otherJob.sessionId, "sess-other");
  const orphanedCompleted = state.jobs.find(
    (job) => job.id === "review-completed",
  );
  assert.equal(orphanedCompleted.sessionId, null);
  const orphanedRunning = state.jobs.find((job) => job.id === "review-running");
  assert.equal(orphanedRunning.sessionId, null);
});
...
test("stop hook runs a stop-time review task and blocks on findings when the review gate is enabled", () => {
  ...
  assert.match(
    ...
    /Gemini stop-time review found issues that still need fixes/i,
  );
  assert.match(blockedPayload.reason, /Missing empty-state guard/i);

  const fakeState = JSON.parse(fs.readFileSync(fakeStatePath, "utf8"));
  assert.match(fakeState.lastInvocation.prompt, /<task>/i);
  assert.match(fakeState.lastInvocation.prompt, /<compact_output_contract>/i);
  assert.match(
    fakeState.lastInvocation.prompt,
...
test("stop hook does not block when Gemini is unavailable even if the review gate is enabled", () => {
  ...
  assert.equal(allowed.status, 0, allowed.stderr);
  assert.equal(allowed.stdout.trim(), "");
  assert.match(allowed.stderr, /Gemini is not set up for the review gate/i);
  assert.match(allowed.stderr, /Run \/gemini:setup/i);
});

test("stop hook does not block when Gemini is not authenticated even if the review gate is enabled", () => {
  const repo = makeTempDir();
  const binDir = makeTempDir();
  ...
  assert.equal(allowed.stdout.trim(), "");
  assert.match(allowed.stderr, /Gemini is not set up for the review gate/i);
  assert.match(allowed.stderr, /No Gemini authentication detected/i);
  assert.match(allowed.stderr, /complete `\/auth`/i);
});
