...
const MODEL_ALIASES = new Map([["spark", "flash"]]);
...
function buildSetupReport(cwd, actionsTaken = []) {
  ...
  const providerStatus = getGeminiAvailability(cwd);
  ...
  if (!providerStatus.available) {
    nextSteps.push(
      `Install ${providerInfo.displayName} CLI with \`${providerInfo.installCommand}\`.`,
    );
  }
  if (providerStatus.available && !authStatus.loggedIn) {
    nextSteps.push(
      "Run `gemini` and complete `/auth`, or set `GEMINI_API_KEY` / `GOOGLE_API_KEY`.",
    );
    nextSteps.push(
      "For Vertex AI, set `GOOGLE_APPLICATION_CREDENTIALS` before rerunning `/gemini:setup`.",
    );
  }
  ...
  return {
    ready:
      nodeStatus.available && providerStatus.available && authStatus.loggedIn,
...
function ensureGeminiReady(cwd) {
  ...
  if (!authStatus.available) {
    throw new Error(
      `${providerInfo.displayName} CLI is not installed or is missing required headless support. Install it with \`${providerInfo.installCommand}\`, then rerun \`/gemini:setup\`.`,
    );
  }
  if (!authStatus.loggedIn) {
    throw new Error(
      `${providerInfo.displayName} CLI is not authenticated. Run \`gemini\` and complete \`/auth\`, or configure \`GEMINI_API_KEY\` / \`GOOGLE_API_KEY\`, then retry.`,
    );
  }
}
...
function validateNativeReviewRequest(target, focusText) {
  ...
  if (!nativeTarget) {
    throw new Error(
      "This `/gemini:review` target is not supported by the built-in reviewer. Retry with `/gemini:adversarial-review` for custom targeting.",
    );
  }
...
async function resolveLatestTrackedTaskThread(cwd, options = {}) {
  ...
  const resolveTrackedTaskResumeToken = (job) => {
    const providerId = String(job?.providerId ?? "gemini")
      .trim()
      .toLowerCase();
    if (typeof job?.resumeToken === "string" && job.resumeToken.trim()) {
      return job.resumeToken.trim();
    }
    return normalizeGeminiResumeToken(job?.threadId);
  };
  ...
  const trackedTask = jobs.find(
    (job) =>
      ...
      resolveTrackedTaskResumeToken(job),
  );
  if (trackedTask) {
    const resumeToken = resolveTrackedTaskResumeToken(trackedTask);
    if (resumeToken) {
      return {
        id: resumeToken,
        threadId: trackedTask.threadId ?? null,
        resumeToken,
      };
    }
  }

  // Fail closed: do not fall back to global-latest session which may belong to a different workspace
  return null;
}
...
function buildReviewJobMetadata(reviewName, target) {
  ...
  return {
    ...
    title:
      reviewName === "Review"
        ? `${providerInfo.displayName} Review`
        : `${providerInfo.displayName} ${reviewName}`,
...
function buildTaskRunMetadata({ prompt, resumeLast = false }) {
  ...
  if (!resumeLast && String(prompt ?? "").includes(STOP_REVIEW_TASK_MARKER)) {
    return {
      title: `${providerInfo.displayName} Stop Gate Review`,
      summary: "Stop-gate review of previous Claude turn",
    };
  }

  const title = resumeLast
    ? `${providerInfo.displayName} Resume`
    : `${providerInfo.displayName} Task`;
    ...
    candidate:
      candidate == null
        ...
        : {
            ...
            resumeToken: candidate.resumeToken ?? null,
  ...
  if (interrupt.attempted) {
    appendLogLine(
      ...
      interrupt.interrupted
        ? `Requested provider turn interrupt for ${turnId} on ${threadId}.`
        : `Provider turn interrupt failed${interrupt.detail ? `: ${interrupt.detail}` : "."}`,
    );
  }
