...
function buildSetupNote(cwd) {
  ...
  const providerName = authStatus?.providerName ?? "Gemini";
  if (authStatus.available && authStatus.loggedIn) {
    return null;
  }

  const detail = authStatus.detail ? ` ${authStatus.detail}.` : "";
  return `${providerName} is not set up for the review gate.${detail} Run /gemini:setup and, if needed, launch \`gemini\` and complete \`/auth\`.`;
}
