...
const STDERR_PREFIX = process.env.GEMINI_COMPANION_PROVIDER_ID || "gemini";
    ...
    return {
      ...
      threadId:
        typeof value.threadId === "string" && value.threadId.trim()
          ...
          : null,
      resumeToken:
        typeof value.resumeToken === "string" && value.resumeToken.trim()
          ? value.resumeToken.trim()
          : null,
      turnId:
...
export function createJobProgressUpdater(workspaceRoot, jobId) {
  ...
  return (event) => {
    ...
    if (normalized.resumeToken && normalized.resumeToken !== lastResumeToken) {
      lastResumeToken = normalized.resumeToken;
      patch.resumeToken = normalized.resumeToken;
      changed = true;
    }
...
export function createProgressReporter({
  ...
  return (eventOrMessage) => {
    ...
    if (stderr && stderrMessage) {
      process.stderr.write(`[${STDERR_PREFIX}] ${stderrMessage}\n`);
    }
...
export async function runTrackedJob(job, runner, options = {}) {
  ...
  try {
    ...
    writeJobFile(job.workspaceRoot, job.id, {
      ...
      resumeToken: execution.resumeToken ?? null,
