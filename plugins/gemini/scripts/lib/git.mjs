...
const SENSITIVE_CONTENT_PATTERNS = [
  /(?:api[_-]?key|apikey|auth[_-]?token|access[_-]?token|secret[_-]?key|bearer|password|passwd|pwd)\s*[=:]\s*["']?[\w-]{8,}/gi,
  /-----BEGIN\s+(?:RSA|EC|DSA|OPENSSH)?\s*PRIVATE\s+KEY-----/,
];

function isSensitivePath(relativePath) {
  const lower = relativePath.toLowerCase();
  return SENSITIVE_FILENAME_PATTERNS.some((pattern) => lower.includes(pattern));
}
...
function formatUntrackedFile(cwd, relativePath) {
  ...
  // Reject symlinks to prevent escaping repo boundary
  const lstat = fs.lstatSync(absolutePath);
  if (lstat.isSymbolicLink()) {
    return `### ${relativePath}\n(skipped: symbolic link)`;
  }

  // Reject files whose real path escapes the repo
  ...
  if (isSensitivePath(relativePath)) {
    return `### ${relativePath}\n(skipped: sensitive filename pattern)`;
  }
  ...
  // Reject files with credential-like content
  if (containsSensitiveContent(content)) {
    return `### ${relativePath}\n(skipped: contains疑似敏感内容)`;
  }
