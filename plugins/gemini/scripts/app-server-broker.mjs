...
import { AppServerClient, BROKER_BUSY_RPC_CODE } from "./lib/app-server.mjs";
...
function getBrokerUserAgent(env = process.env) {
  const providerName = getProviderName(env)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-");
  return `${providerName}-companion-broker`;
}
...
async function main() {
  ...
  const pidFile = options["pid-file"]
    ? path.resolve(options["pid-file"])
    : null;
  writePidFile(pidFile);

  const appClient = await AppServerClient.connect(cwd, { disableBroker: true });
      ...
      while (newlineIndex !== -1) {
        ...
        if (message.id !== undefined && message.method === "initialize") {
          send(socket, {
            id: message.id,
            result: {
              userAgent: getBrokerUserAgent(process.env),
            },
          });
          continue;
        }
        ...
        if (
          ...
          send(socket, {
            ...
            error: buildJsonRpcError(
              ...
              `Shared ${getProviderName(process.env)} compatibility broker is busy. Wait for the current request to finish or retry through the direct Gemini-backed path.`,
            ),
          });
          continue;
        }
