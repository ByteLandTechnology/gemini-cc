...
- For `gemini:rescue`, do not turn a failed or incomplete Gemini run into a Claude-side implementation attempt. Report the failure and stop.
- For `gemini:rescue`, if Gemini was never successfully invoked, do not generate a substitute answer at all.
